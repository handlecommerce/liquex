defmodule Liquex.ArgumentTest do
  ...
  describe "eval" do
    test "evaluate literal" do
      assert {5, _} = Argument.eval([literal: 5], Context.new(%{}))
    end

    test "evaluate unknown field" do
      assert {nil, _} = Argument.eval([field: [key: "i"]], Context.new(%{}))
      assert {nil, _} = Argument.eval([field: [key: "a", key: "b"]], Context.new(%{}))
    end

    test "evaluate with known field" do
      assert {5, _} = Argument.eval([field: [key: "i"]], Context.new(%{"i" => 5}))

      assert {5, _} =
               Argument.eval([field: [key: "a", key: "b"]], Context.new(%{"a" => %{"b" => 5}}))
    end

    test "evaluate with atom keys" do
      assert {5, _} = Argument.eval([field: [key: "i"]], Context.new(%{i: 5}))
      assert {5, _} = Argument.eval([field: [key: "a", key: "b"]], Context.new(%{a: %{b: 5}}))
    end
    ...
    test "evaluate with array.first" do
      ...
      assert {5, _} = Argument.eval([field: [key: "field", key: "first", key: "child"]], obj)
    end

    test "evaluate with array.last" do
      obj = Context.new(%{"field" => [1, 2, 3, 4, 5]})

      assert {5, _} = Argument.eval([field: [key: "field", key: "last"]], obj)
    end

    test "evaluate with array.size" do
      obj = Context.new(%{"field" => [1, 2, 3, 4, 5]})

      assert {5, _} = Argument.eval([field: [key: "field", key: "size"]], obj)
    end

    test "evaluate with string.size" do
      obj = Context.new(%{"field" => "String"})

      assert {6, _} = Argument.eval([field: [key: "field", key: "size"]], obj)
    end
    ...
    test "anonymous function field" do
      ...
      assert {5, _} = Argument.eval([field: [key: "field"]], obj)
    end
  end
  ...
  describe "assign" do
    test "assign new value" do
      ...
      assert {5, _} = Argument.eval([field: [key: "i"]], context)
    end
    ...
    test "assign new nested value" do
      ...
      assert {5, _} = Argument.eval([field: [key: "a", key: "b"]], context)
    end
    ...
    test "assign in list" do
      ...
      assert {[1, 9, 3], _} = Argument.eval([field: [key: "field"]], context)
    end
  end
end
